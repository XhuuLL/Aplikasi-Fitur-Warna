...
st.set_page_config(page_title="Deteksi Buah", page_icon="🍎")
st.markdown("""
    <style>
...
@st.cache_resource
def load_model():
    return joblib.load("model_buah.pkl")

try:
    model_buah = load_model()
except FileNotFoundError:
    st.error("File 'model_buah.pkl' tidak ditemukan. Jalankan 'python train_model.py' terlebih dahulu.")
    st.stop()
...
with tab1:
    uploaded_file = st.file_uploader("Unggah Foto Buah (JPG/PNG)", type=["jpg", "jpeg", "png"])
with tab2:
...
if image_source is not None:
    ...
    with st.spinner("Mengekstrak histogram warna dan menganalisis..."):
        ...
        prediksi = model_buah.predict([fitur])[0]
        
        # Desain Card Custom menggunakan HTML & CSS
        html_result = f"""
        <div style="
            background: linear-gradient(135deg, rgba(255, 75, 75, 0.15) 0%, rgba(255, 255, 255, 0) 100%);
        ...
        ">
            ...
            <h1 style="
                ...
                letter-spacing: 3px; 
                font-weight: 900; 
                text-shadow: 2px 2px 8px rgba(255, 75, 75, 0.3);
            ">
                {prediksi}
            </h1>
            <hr style="border: none; border-top: 1px solid rgba(255, 75, 75, 0.2); margin: 20px 0 15px 0;">
